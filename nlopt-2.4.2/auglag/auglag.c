...
nlopt_result auglag_minimize (int n, nlopt_func f, void *f_data, int m,
                              nlopt_constraint *fc, int p, nlopt_constraint *h,
                              ...
                              nlopt_opt sub_opt, int sub_has_fc)
{
    ...
    ret = nlopt_set_min_objective (sub_opt, auglag, &d);
    if (ret < 0) return ret;
    ret = nlopt_set_lower_bounds (sub_opt, lb);
    if (ret < 0) return ret;
    ret = nlopt_set_upper_bounds (sub_opt, ub);
    if (ret < 0) return ret;
    ret = nlopt_set_stopval (sub_opt,
                             d.m == 0 && d.p == 0 ? stop->minf_max : -HUGE_VAL);
    if (ret < 0) return ret;
    ret = nlopt_remove_inequality_constraints (sub_opt);
    if (ret < 0) return ret;
    ret = nlopt_remove_equality_constraints (sub_opt);
    if (ret < 0) return ret;
    for (i = 0; i < m; ++i) {
        if (fc[i].f)
            ret = nlopt_add_inequality_constraint (sub_opt, fc[i].f,
                                                   fc[i].f_data, fc[i].tol[0]);
        else
            ret = nlopt_add_inequality_mconstraint (sub_opt, fc[i].m, fc[i].mf,
                                                    fc[i].f_data, fc[i].tol);
        if (ret < 0) return ret;
    }

    xcur = (double *)malloc (
        sizeof (double) * (n + max_constraint_dim * (1 + n) + d.pp + d.mm));
    ...
    memset (d.gradtmp, 0,
            sizeof (double) * (n * max_constraint_dim + d.pp + d.mm));
    ...
    *minf = HUGE_VAL;
    ...
    do {
        ...
        ret = nlopt_optimize_limited (
            sub_opt, xcur, &fcur, stop->maxeval - stop->nevals,
            stop->maxtime - (nlopt_seconds () - stop->start));
        if (auglag_verbose) printf ("auglag: subopt return code %d\n", ret);
        if (ret < 0) break;
        ...
        if (auglag_verbose) printf ("auglag: fcur = %g\n", fcur);
        ...
        if ((feasible
             && (!minf_feasible || penalty < minf_penalty || fcur < *minf))
